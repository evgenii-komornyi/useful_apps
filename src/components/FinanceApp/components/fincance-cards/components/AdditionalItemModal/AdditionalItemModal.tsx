...
export const AdditionalItemModal = (): ReactElement => {
    const {isOpened, type, budgetDate, setIsOpened} = useModalStore(state => state);
    ...
    const onChangeHandler = ({target: {name, value}}: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    const addItemHandler = () => {
        if (type === ProfitExpenseType.Profit) {
            addAdditionalItemToBudget(fields as Profit, ProfitExpenseType.Profit, budgetDate);
        } else {
            addAdditionalItemToBudget(fields as Expense, ProfitExpenseType.Expenses, budgetDate);
        }
        closeModal();
    ...
    return (
        ...
        >
            ...
            <DialogContent dividers>
                <AdditionalForm type={type} fields={fields} onChangeHandler={onChangeHandler} />
            </DialogContent>
            <DialogActions>
                <Button variant="outlined" onClick={addItemHandler}>Add {withoutLastLetterIfTypeExpenses(type)}</Button>
