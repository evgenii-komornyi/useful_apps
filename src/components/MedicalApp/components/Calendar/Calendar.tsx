...
function ServerDay(
    props: PickersDayProps<Dayjs> & { highlightedDays?: HState[] },
) {
    const { highlightedDays = [], day, outsideCurrentMonth, ...other } = props;
    const isSelected =
        !props.outsideCurrentMonth &&
        highlightedDays.findIndex(
            item =>
                item.day === props.day.date() &&
                item.month === props.day.month() &&
                item.year === props.day.year(),
        ) >= 0;
...
export const Calendar: FC<Props> = ({
    ...
    const fetchHighlightedDays = (date: Dayjs) => {
        const anamnesisItem = anamnesis.find(
            anamnesisItem =>
                anamnesisItem.year === date.year() &&
                anamnesisItem.month === date.month(),
        );
        setHighlightedDays(
            anamnesisItem?.symptoms.map(({ date: { day, month, year } }) => ({
                day,
                month,
                year,
            })) || [],
    ...
    const onMonthChangeHandler = (newDate: Dayjs) => {
        ...
        onMonthChange(
            anamnesis.find(
                anamnesisItem =>
                    anamnesisItem.year === newDate.year() &&
                    anamnesisItem.month === newDate.month(),
            ),
        );
    };
